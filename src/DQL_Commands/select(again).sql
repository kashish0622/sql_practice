...
RENAME COLUMN teacher_id TO teacher_assigned;
...
SELECT marks, count(teacher_assigned) FROM student_info
GROUP BY marks;

SELECT marks, count(teacher_assigned) FROM student_info
GROUP BY marks HAVING marks > 50;

SELECT name, marks, count(teacher_assigned) FROM student_info
GROUP BY name, marks HAVING marks > 50 ORDER BY name ASC;